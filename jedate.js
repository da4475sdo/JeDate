...
(function (root, factory) {
    ...
    var QD = function () {
        ...
        function p(c, g, d) {
            ...
            for (var b = [], e = -1, f = (a.match(t) || k)[1], h = !f && (a.match(u) || k)[1], a = !f && (a.match(v) || k)[1], m = -1, q, l, n, a = a && a.toLowerCase(); q = g[++m];) {
                l = q.parentNode;
                do
                    if (n = (n = (n = !a || "*" === a || a === l.nodeName.toLowerCase()) && (!f || l.id === f)) && (!h || RegExp("(^|\\s)" + h + "(\\s|$)").test(l.className)), d || n)
                        break;
    ...
    jeDt.parent = function (target, id) {
        var parent = target;
        while (parent.id && parent.id != id) {
    ...
    jeDt.initDate = function (opts) {
        ...
        jeDt.isYYMM = jeDt.parseCheck(opts.format || config.format, true) == "YYYY-MM" ? true : false;
        jeDt.ishhmm = jeDt.parseCheck(opts.format || config.format, false).substring(0, 5) == "hh:mm" ? true : false;
        jeDt.opts = opts;
        if (opts.isDouble) {
            if (QD("#double-date")[0]) return;

            var date = jeDt.initHiddenInDouble(opts);

            var str = '<input id="date-value1" type="hidden" value="' + (date[0] || '') + '" class="datep" style="position: absolute;right: 0; top: 0">\
                <input id="date-value2" type="hidden" value="' + (date[1] || '') + '" class="datep" style="position: absolute; right: 0;">\
                <div class="discls" style="float: left;">\
                </div>\
                <div class="discls" style="float: left;">\
                </div>';
            jeDt.container = doc.createElement('div');
            jeDt.container.id = 'double-date';
            jeDt.container.innerHTML = str;
            doc.body.appendChild(jeDt.container);

        } else {
            if (!jeDt.defaultDate[opts.dateCell]) {
                jeDt.defaultDate[opts.dateCell] = $(opts.dateCell).val();
            }
        }
        ...
        var initVals = function (elem) {
            ...
            if (opts.isDouble) {
                jeDt.setDateInDouble(opts);
            }
        };
        var valcell = QD(opts.dateCell);
    ...
    jeDt.initHiddenInDouble = function (opts) {
        var dateCell = opts.dateCell2;

        var date = $(dateCell).val().split(jeDt.trim(opts.joinChar));
        var _tmp = [];
        if (date.length >= 4) {
            _tmp[0] = jeDt.trim(date[0]) + '-' + jeDt.trim(date[1]);
            _tmp[1] = jeDt.trim(date[2]) + '-' + jeDt.trim(date[3]);
        }
        if (date.length >= 6) {
            _tmp[0] += '-' + jeDt.trim(date[2]);
            _tmp[1] = jeDt.trim(date[3]) + '-' + jeDt.trim(date[4] + '-' + jeDt.trim(date[5]));
        }
        if (date.length > 2) {
            date = [];
            date.push(_tmp[0]);
        ...
        if (!jeDt.defaultDate[dateCell]) {
            jeDt.defaultDate[dateCell] = [];
            jeDt.defaultDate[dateCell][0] = date[0];
            jeDt.defaultDate[dateCell][1] = date[1];
        }
    ...
    jeDt.setHtml = function (opts, self, lyerCell) {
        var that = this, weekHtml = "", tms = "", date = new Date(), nowDateVal = jeDt.nowDate(null, opts.format || config.format),
            ...
            initVal = opts.isinitVal ? jeDt.isValHtml(self) ? jeDt.val(self) : jeDt.text(self) : (jeDt.val(self) ||
            jeDt.text(self)) == "" ? nowDateVal : jeDt.isValHtml(self) ? jeDt.val(self) : jeDt.text(self);
        ...
        if (opts.isDouble) {
            jeDt.isShow(QD('#double-date .jedatebtn')[0], false);
        }

        jeDt.each(QD(lyerCell + " .jedatebot .jedateclear"), function (i, elem) {
            jeDt.isShow(elem, opts.isClear);
        });
        if (isTimeFormat) {
        ...
        } else if (opts.isDouble) {
            jeDt.orien(jeDt.container, QD(opts.dateCell2)[0]);
        }
    ...
    jeDt.onlyYMStr = function (opts, y, m) {
        ...
        jeDt.each(jeDt.montharr, function (i, val) {
            var minArr = jeDt.parseMatch(opts.minDate || config.minDate), maxArr = jeDt.parseMatch(opts.maxDate || config.maxDate), thisDate = new Date(y, jeDt.digit(val), "01"),
                minTime = new Date(minArr[0] || new Date().getFullYear(), minArr[1] || 1, minArr[2] || 1),
                maxTime = new Date(maxArr[0] || new Date().getFullYear(), maxArr[1] || 1, maxArr[2] || 1);
            if (thisDate < minTime || thisDate > maxTime) {
                onlyYM += "<li title='" + y + "-" + jeDt.digit(val) + "' class='disabled' " + (m == val ? 'class="action"' : "") + ' data-onym="' + y + "-" + jeDt.digit(val) + '">' + y + "年" + jeDt.digit(val) + "月</li>";
            } else {
                onlyYM += "<li title='" + y + "-" + jeDt.digit(val) + "'" + (m == val ? 'class="action"' : "") + ' data-onym="' + y + "-" + jeDt.digit(val) + '">' + y + "年" + jeDt.digit(val) + "月</li>";
            }
        });
        return onlyYM;
    };
    jeDt.onlyYMevents = function (opts, self, lyerCell, tms) {
        ...
        var date = QD(lyerCell + ' .jedaym .action')[0].innerHTML;
        jeDt.each([ymPre, ymNext], function (i, cls) {
            jeDt.bind(cls, "click", function (ev) {
                if (jeDt.hasClass(cls[0], "disabled"))
                    return;
                jeDt.stopmp(ev);
                var ym = cls == ymPre ? ony -= 1 : ony += 1;
                jeDt.html(QD(lyerCell + " .jedaym")[0], jeDt.onlyYMStr(opts, ym, onm));

                var action = QD(lyerCell + ' .jedaym .action')[0];
                action && (action.className = '');
                var li = QD(lyerCell + ' .jedaym li');
                for (var i = 0; i < li.length; i++) {
                    if (!jeDt.hasClass(li[i], 'disabled')) {
                        date = jeDt.trim(li[i].innerHTML);
                        jeDt.addClass(li[i], 'action');
                        break;
                    }
                }
                jeDt.text(QD(lyerCell + ' .jedateyearmonth')[0], date);
                jeDt.events(opts, self, lyerCell, tms);
            });
        });
    };
    ...
    jeDt.orien = function (obj, self, pos) {
        ...
        var leris = rect.right + obj.offsetWidth / 1.5 >= jeDt.winarea(1) ? rect.right - obj.offsetWidth : rect.left + (pos ? 0 : jeDt.docScroll(1));
    ...
    jeDt.getDateStr = function (opts, lyerCell, y, m, d) {
        ...
        for (var p = pervLastDay - 1; p >= 0; p--) {
            ...
            dayStr += "<li title='" + preDays + "' class='" + preCls + "' data-y='" + py + "' data-m='" + jeDt.digit(parseInt(pm) - 1) + "' data-d='" + preDays + "'>" + isfestival(jeDt.digit(parseInt(pm) - 1) + "." + preDays, preDays) + mks(py, parseInt(pm) - 1, preDays) + "</li>";
        }
        ...
        for (var j = startDay; j <= endDay; j++) {
            ...
            dayStr += '<li title="' + j + '" class="' + current + '" data-y="' + y + '" data-m="' + m + '" data-d="' + j + '">' + isfestival(m + "." + j, j) + mks(y, m, j) + "</li>";
        }
        for (var k = endDay + 1; k <= currentMonthDays; k++) {
            k = jeDt.digit(k);
            dayStr += '<li title="' + k + '" class="disabled" data-y="' + y + '" data-m="' + m + '" data-d="' + k + '">' + isfestival(m + "." + k, k) + mks(y, m, k) + "</li>";
        }
        //循环补上下个月的开始几天
    ...
    jeDt.events = function (opts, self, lyerCell, tms) {
        var isYYMM = jeDt.isYYMM;
        var ishhmmss = jeDt.ishhmm;
        ...
        if (!isYYMM) {
            ...
            if (ishhmmss) {
                ...
                jeDt.bind(QD(lyerCell + " .jedatehms"), "click", function () {
                    if (QD(lyerCell + " .jedateprophms")[0].style.display !== "block")
                        hmsevents([hs, ms, ss]);
                    //关闭时分秒层
            ...
            jeDt.bind(QD(lyerCell + " .jedatebot .jedatetodaymonth"), "click", function () {
                ...
                if (opts.isDouble) {
                    jeDt.getDateStr(opts, jeDt.lyerCell[0], toTime[0], toTime[1], toTime[2]);
                    jeDt.getDateStr(opts, jeDt.lyerCell[1], toTime[0], toTime[1], toTime[2]);
                    jeDt.val(QD('#date-value1')[0], gettoDate);
                    jeDt.val(QD('#date-value2')[0], gettoDate);
                    jeDt.dateClose(opts, jeDt.lyerCell[0]);
                    jeDt.dateClose(opts, jeDt.lyerCell[1]);
                    if (jeDt.isType(opts.choosefun, "function") || opts.choosefun != null)
                        opts.choosefun(gettoDate);
                    if (!isYYMM) {
                        jeDt.chooseDays(opts, self, jeDt.lyerCell[0]);
                        jeDt.chooseDays(opts, self, jeDt.lyerCell[1]);
                    }
                    jeDt.setDateInDouble(opts);
                    doc.body.removeChild(jeDt.container);
                } else {
            ...
            jeDt.bind(QD(lyerCell + " .jedaym li"), "click", function (ev) {
                ...
                ev.target.parentNode.querySelector('.action').className = '';
                ev.target.className = 'action';
                jeDt.isValHtml(valcell) ? jeDt.val(valcell, getYMDate) : jeDt.text(valcell, getYMDate);
                jeDt.dateClose(opts, lyerCell);
                if (jeDt.isType(opts.choosefun, "function") || opts.choosefun != null)
                    opts.choosefun(getYMDate);
            });
            //本月
            jeDt.bind(QD(lyerCell + " .jedatebot .jedatetodaymonth"), "click", function () {
                ...
                if (opts.isDouble) {
                    jeDt.val(QD('#date-value1')[0], YMDate);
                    jeDt.val(QD('#date-value2')[0], YMDate);
                    jeDt.setDateInDouble(opts);
                    doc.body.removeChild(jeDt.container);
                } else {
        ...
        jeDt.bind(QD(lyerCell + " .jedatebot .jedateclear"), "click", function () {
            if (opts.isDouble) {
                $(opts.dateCell2).val('').trigger('change');
                var start = $('#date-value1');
                var end = $('#date-value2');
                start.val('');
                end.val('');
                jeDt.setDateInDouble(opts);
                if (jeDt.isType(opts.clearfun, "function") || opts.clearfun != null) {
                    opts.clearfun({
                        start: start.val(),
                        end: end.val()
                    });
        ...
        if (!isYYMM)
            !ishhmmss && jeDt.chooseDays(opts, self, lyerCell);
    };
    //setDateInDouble方法添加参数start,end
    jeDt.setDateInDouble = function (opts,start,end) {
        var dateCell = QD(opts.dateCell2)[0],
            dateEnd = end?end:QD('#date-value2')[0].value,
            dateStart =start?start: QD('#date-value1')[0].value;
        if (new Date(dateEnd) < new Date(dateStart)) {
            alert('结束日期必须大于开始日期');
            return false;
        }
        if (jeDt.trim(dateStart) != '') {
            dateCell.value = jeDt.trim(dateStart) + opts.joinChar + jeDt.trim(dateEnd);
        } else {
            dateCell.value = '';
        }
        $(dateCell).siblings('[data-date="end"]').val(jeDt.trim(dateEnd));
        $(dateCell).siblings('[data-date="start"]').val(jeDt.trim(dateStart));
        $(opts.dateCell2).trigger('change');
        return true;
    };
    //移除信息
    var once = 0;
    jeDt.removeBox = function () {
        if (once !== 0)
            return;
        once++;
        //点击空白处隐藏
        jeDt.bind(document, "mouseup", function (ev) {
            if (!jeDt.opts.isDisplay) {
                jeDt.stopmp(ev);
                var box = QD(jeDt.lyerCell[0])[0];
                if (box && box.style.display !== "none")
                    doc.body.removeChild(box);
            }

            if (jeDt.opts.isDouble) {
                jeDt.stopmp(ev);
                var remove = QD('#double-date')[0];
                if (remove) {
                    doc.body.removeChild(remove);
                }
            }
        });
    };
    //确认
    jeDt.bindOkBtn = function () {
        var valCell = QD(jeDt.opts.dateCell);
        var opts = jeDt.opts;
        var isYYMM = jeDt.isYYMM;
        var ishhmmss = jeDt.ishhmm;
        jeDt.each(QD(".jedatebox"), function (i, obj) {
            var lyerCell = '#' + obj.id;
            var jedateyear = QD(lyerCell + " .jedateyear"),
                jedatemonth = QD(lyerCell + " .jedatemonth");
            jeDt.bind(QD(lyerCell + " .jedatebot .jedateok"), "click", function (ev) {
                jeDt.stopmp(ev);
                var valcell = valCell[i],
                    ...
                    preEachhmsem=function (okArr,preLyerCell){
                        jeDt.each(QD(preLyerCell + " .jedatehms em"), function (l, emval) {
                            okArr.push(jeDt.text(emval));
                        });
                    };
                ...
                if (isValtext) {
                    var jedCell = isYYMM ? QD(lyerCell + " .jedaym li") : QD(lyerCell + " .jedaul li"),
                        //前一个时间选择框的ID
                        preLyerCell="#"+obj.id.substring(0,6)+(parseInt(obj.id.substring(6))-1),
                    ...
                    if (!isYYMM) {
                        var isCorrectTimes=true;
                        //如果是双框时间选择器
                        if(opts.isDouble){
                            //前一个时间选择框的完整值
                            var preOkVal="";
                            //前一个时间选择框的年
                            var preJedateyear=QD(preLyerCell + " .jedateyear");
                            //前一个时间选择框的月
                            var preJedatemonth=QD(preLyerCell + " .jedatemonth");
                            //前一个时间选择框的年月日数组
                            var preOktms=jeDt.isValHtml(valCell[i-1]) ? jeDt.val(valCell[i-1]).match(/\w+|d+/g) : jeDt.text(valCell[i-1]).match(/\w+|d+/g);
                            var okTimeArr = [],preOkTimeArr=[], okTime = [parseInt(jeDt.attr(jedateyear[0], "data-year")), parseInt(jeDt.attr(jedatemonth[0], "data-month")), oktms[2]];
                            //获取后一个时间选择框的值
                            eachhmsem(okTimeArr);
                            //获取前一个时间选择框的值
                            preEachhmsem(preOkTimeArr,preLyerCell);
                            okVal = isValtext ? jeDt.parse([okTime[0], okTime[1], okTime[2]], [okTimeArr[0], okTimeArr[1], okTimeArr[2]], opts.format || config.format) : "";
                            preOkVal=isValtext ? jeDt.parse([preOktms[0],preOktms[1], preOktms[2]], [preOkTimeArr[0], preOkTimeArr[1], preOkTimeArr[2]], opts.format || config.format) : "";
                            okVal=preOkVal+";"+okVal;
                        ...
                        var jedYM = isValtext ? jeDt.attr(QD(lyerCell + " .jedaym .action")[0], "data-onym").match(/\w+|d+/g) : "",
                            preJedYM=isValtext ? jeDt.attr(QD(preLyerCell + " .jedaym .action")[0], "data-onym").match(/\w+|d+/g) : "",
                            preOkVal=isValtext ? jeDt.parse([preJedYM[0], preJedYM[1], 1], [0, 0, 0], opts.format || config.format) : "";
                    ...
                    if(!opts.isDouble){
                        ...
                        //获取起始和结束时间数组
                        var times=okVal.split(";");
                        //判断起始时间和结束时间是否正确
                        isCorrectTimes=jeDt.setDateInDouble(opts,times[0],times[1]);
                        if(isCorrectTimes){
                            //将双框时间选择器的数据显示到页面上
                            if(times!=null&&times.length!=0){
                                ...
                                //用于提交的隐藏起始时间
                                var start=$("input[data-date='start']").val(startTime);
                                //用于提交的隐藏结束时间
                                var end=  $("input[data-date='end']").val(endTime);
                                //将起始和结束时间赋值给输入框
                                $("#form").val(startTime+" - "+endTime);
                            }
                            //关闭双框时间选择框
                            doc.body.removeChild(QD("#double-date")[0]);
                        }
                    }
                    if (okVal != "") {
                        if (jeDt.isType(opts.okfun, "function") || opts.okfun != null)
                            opts.okfun(okVal);
                    }
                    if (!isYYMM)
                        jeDt.chooseDays(opts, self, lyerCell);
    ...
    var jeDate = function (options) {
        ...
        options.isDisplay = false;
        options.festival = true; //显示节日
        if (options.isDouble) {
            options.isDisplay = true;
            options.dateCell2 = options.dateCell;
            options.displayCell = "div.discls";
            options.dateCell = 'input.datep';
            options.joinChar = options.joinChar || ' - ';
            options.tool = false;
        }
